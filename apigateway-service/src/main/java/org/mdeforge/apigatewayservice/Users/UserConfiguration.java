    ...
    RouteLocator gatewayUserServiceRouters(RouteLocatorBuilder builder){
        ...
        return builder.routes()
                ...
                .route(r -> r.path("/user/id/**").and().method("GET")
                        .filters(f -> f.rewritePath("/user/id/(?<userId>.*)", "/findUser/%7BuserId%7D?id=${userId}"))
                        .uri(userservice_url))
                .route(r -> r.path("/user/email/**").and().method("GET")
                        .filters(f -> f.rewritePath("/user/email/(?<email>.*)", "/findUserByEmail/%7Bemail%7D?email=${email}"))
                        .uri(userservice_url))
                .route(r -> r.path("/user/username/**").and().method("GET")
                        .filters(f -> f.rewritePath("/user/username/(?<username>.*)", "/findUserByUsername/%7Busername%7D?username=${username}"))
                        .uri(userservice_url))
                ...
                .route(r -> r.path("/view/user/id/**").and().method("GET")
                        .filters(f -> f.rewritePath("/view/user/id/(?<userId>.*)", "/findUser/%7BuserId%7D?id=${userId}"))
                        .uri(mdeforgeviewservice_url))
                .route(r -> r.path("/view/user/email/**").and().method("GET")
                        .filters(f -> f.rewritePath("/view/user/email/(?<email>.*)", "/findUserByEmail/%7Bemail%7D?email=${email}"))
                        .uri(mdeforgeviewservice_url))
                .route(r -> r.path("/view/user/username/**").and().method("GET")
                        .filters(f -> f.rewritePath("/view/user/username/(?<username>.*)", "/findUserByUsername/%7Busername%7D?username=${username}"))
                        .uri(mdeforgeviewservice_url))
                .build();
    }
